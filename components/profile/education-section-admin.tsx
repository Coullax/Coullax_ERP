...
import { getEmployeeEducation, addEducation, deleteEducation, verifyEmployeeEducation } from '@/app/actions/profile-actions'
import { Plus, Trash2, GraduationCap, CheckCircle, Shield } from 'lucide-react'
import { useAuthStore } from '@/store/auth-store'

interface EducationSectionAdminProps {
    employeeId: string
    employee?: any
}
...
export function EducationSectionAdmin({ employeeId, employee }: EducationSectionAdminProps) {
    ...
    // Check if all education records are verified
    const isEducationVerified = education.length > 0 && education.every(edu => edu.isverified === true)

    const loadEducation = useCallback(async () => {
        try {
            console.log('Loading education for employeeId:', employeeId)
    ...
    const handleVerify = async () => {
        ...
        if (education.length === 0) {
            toast.error('No education records to verify')
            return
        }

        setVerifying(true)
        try {
            await verifyEmployeeEducation(employeeId, user.id)
                    ...
                    <div className="flex gap-2">
                        ...
                        {education.length > 0 && (
                            <Button
                                ...
                                disabled={verifying}
                                size="sm"
                                className={isEducationVerified
                                    ? "bg-blue-600 hover:bg-blue-700"
                                    : "bg-green-600 hover:bg-green-700"
                                }
                            >
                                {verifying ? (
                                    isEducationVerified ? 'Re-verifying...' : 'Verifying...'
                                ) : (
                                    <>
                                        <CheckCircle className="w-4 h-4 mr-2" />
                                        {isEducationVerified ? 'Re-verify Education' : 'Verify Education'}
                                    </>
                                )}
                            </Button>
                        )}
